...
class AirtableOAuthProvider(BaseOAuthProvider):
    ...
    async def ensure_valid_token(self) -> bool:
        ...
        if self.access_token and not self.refresh_token:
            logger.debug("Access token provided without refresh token - assuming valid for direct usage")
            return True
        ...
        logger.warning("Unable to ensure valid token - no refresh token available or refresh failed")
        return False
    ...
    def get_authorization_url(
        ...
        **kwargs
        ...
        return self.client.create_authorization_url(
            self.config.get_authorization_url(),
            **auth_params
    ...
    async def exchange_code_for_tokens(
        ...
        try:
            logger.info(f"Exchanging authorization code for tokens with Airtable")
            ...
            token = await self.client.fetch_token(self.config.get_token_url(), **token_params)
            ...
            logger.info(f"✅ OAuth token exchange successful! Access token expires in {expires_in}s.")
            return True, token
