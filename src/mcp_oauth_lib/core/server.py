...
class MCPOAuthServer:
    ...
    def __init__(
        ...
        logger.info(f"Initialized OAuth server for provider: {provider.config.provider_name}")
    ...
    def register_oauth_endpoints(self, mcp_app: FastMCP) -> None:
        ...
        @mcp_app.custom_route("/.well-known/oauth-authorization-server", methods=["GET", "OPTIONS"])
        async def oauth_metadata(request: Request) -> JSONResponse:
            """OAuth authorization server metadata discovery endpoint."""
            logger.debug(f"=== METADATA REQUEST ===")
            ...
            try:
                ...
                logger.debug(f"Returning OAuth metadata for provider: {self.provider.config.provider_name}")
                
                return self._create_cors_response(metadata)
            except Exception as e:
                logger.error(f"Failed to get OAuth metadata: {e}")
                raise HTTPException(status_code=500, detail="Failed to get OAuth metadata")
        
        # Authorization initiation endpoint
        @mcp_app.custom_route("/auth/authorize", methods=["GET"])
        async def authorize(request: Request) -> RedirectResponse:
            """Initiate OAuth authorization flow."""
            logger.debug(f"🚨 AUTHORIZATION ENDPOINT CALLED! 🚨")
            logger.debug(f"Method: {request.method}")
            logger.debug(f"URL: {request.url}")
        ...
        async def token_exchange(request: Request) -> JSONResponse:
            ...
            try:
                ...
                logger.debug(f"Token exchange request - code: {code[:10]}..., code_verifier: {'provided' if code_verifier else 'missing'}")
                
                response = await self.oauth_flow.exchange_code_for_tokens(code, code_verifier)
        ...
        async def refresh_token(request: Request) -> JSONResponse:
            ...
            try:
                success, token_data = await self.provider.refresh_access_token(refresh_token_value)
                
                if not success or not token_data:
                    return JSONResponse(
                        status_code=400,
                        content={"error": "refresh_failed", "error_description": "Failed to refresh access token"}
                    )
        ...
        async def introspect_token(request: Request) -> JSONResponse:
            ...
            try:
                ...
                return JSONResponse(
                    status_code=500,
                    content={"error": "introspection_error", "error_description": str(e)}
                )
        ...
        async def revoke_token(request: Request) -> JSONResponse:
            ...
            try:
                ...
                if not success:
                    return JSONResponse(
                        status_code=400,
                        content={"error": "revocation_failed", "error_description": "Failed to revoke token"}
                    )
                
                return JSONResponse(content={"status": "success", "message": "Token revoked successfully"})
        ...
        @mcp_app.custom_route("/.well-known/oauth-protected-resource", methods=["GET", "OPTIONS"])
        async def oauth_protected_resource(request: Request) -> JSONResponse:
            ...
            try:
                ...
                raise HTTPException(status_code=500, detail="Failed to get OAuth protected resource metadata")
        
        # MCP-specific OAuth protected resource endpoint
        @mcp_app.custom_route("/.well-known/oauth-protected-resource/mcp", methods=["GET", "OPTIONS"])
        async def mcp_oauth_protected_resource(request: Request) -> JSONResponse:
            ...
            try:
                ...
                logger.error(f"Failed to get MCP OAuth protected resource metadata: {e}")
                raise HTTPException(status_code=500, detail="Failed to get MCP OAuth protected resource metadata")
        
        # OAuth Authorization Server metadata with MCP extension
        @mcp_app.custom_route("/.well-known/oauth-authorization-server/mcp", methods=["GET", "OPTIONS"])
        async def oauth_authorization_server_mcp(request: Request) -> JSONResponse:
            ...
            try:
                ...
                logger.error(f"Failed to get MCP OAuth authorization server metadata: {e}")
                raise HTTPException(status_code=500, detail="Failed to get MCP OAuth authorization server metadata")
        ...
        async def register_client(request: Request) -> JSONResponse:
            ...
            try:
                ...
                client_info = {
                    ...
                    "grant_types": registration_request.get("grant_types", ["authorization_code", "refresh_token"]),
        ...
        async def oauth_app_info(request: Request) -> JSONResponse:
            ...
            try:
                ...
                raise HTTPException(status_code=500, detail="Failed to get OAuth app info")
        
        logger.info(f"OAuth endpoints registered successfully for provider: {self.provider.config.provider_name}")
    ...
    async def _parse_token_request(self, request: Request) -> tuple[Optional[str], Optional[str]]:
    ...
    async def _extract_token_parameter(self, request: Request, param_name: str) -> Optional[str]:
    ...
    def _create_cors_response(self, content: Dict[str, Any], status_code: int = 200) -> JSONResponse:
