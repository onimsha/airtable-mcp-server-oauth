...
class OAuthFlow:
    ...
    async def initiate_authorization(
        ...
        try:
            ...
            if client_code_challenge and client_code_challenge_method:
                ...
                provider_code_verifier, provider_code_challenge = (
                    generate_provider_compatible_pkce(
                ...
                logger.debug("✅ Generated provider-compatible PKCE:")
                logger.debug(
                    f"   - Client code_challenge: {client_code_challenge[:10]}..."
                )
                logger.debug(
                    f"   - Provider code_verifier: {provider_code_verifier[:10]}..."
                )
                logger.debug(
                    f"   - Provider code_challenge: {provider_code_challenge[:10]}..."
                )
            ...
            logger.debug("✅ Using provider-compatible PKCE with backend")

            logger.info(
                f"Initiating OAuth authorization for user_id={user_id}, state={state}"
            )
            ...
            raise HTTPException(
                status_code=500, detail="Failed to initiate OAuth authorization"
            )

    async def handle_callback(
        self, request: Request, code: str, state: str, error: str | None = None
    ) -> JSONResponse:
        ...
        try:
            ...
            auth_code_data = {
                ...
                "client_code_challenge_method": state_info.get(
            ...
            logger.info(
                "✅ Authorization successful! Stored authorization code for token exchange."
            )
            ...
            if client_redirect_uri:
                ...
                if state_info.get("client_code_challenge"):
                    redirect_params["code_challenge"] = state_info[
                        "client_code_challenge"
                    ]
                    redirect_params["code_challenge_method"] = state_info[
                        "client_code_challenge_method"
                    ]

                redirect_url = f"{client_redirect_uri}?{urlencode(redirect_params)}"
                logger.info(
                    f"Redirecting back to client with authorization code: {redirect_url}"
                )
    ...
    async def exchange_code_for_tokens(
        ...
        try:
            ...
            if code not in self._active_codes:
                logger.error(
                    f"❌ Invalid or expired authorization code: {code[:10]}..."
                )
                return JSONResponse(
                    status_code=400,
                    content={
            ...
            if client_code_challenge and client_code_challenge_method:
                ...
                is_valid = validate_pkce(
                    code_verifier, client_code_challenge, client_code_challenge_method
                )
                if not is_valid:
            ...
            logger.info(
                f"✅ Token exchange successful for user_id={auth_data.get('user_id')}"
            )
    ...
    def cleanup_expired_entries(self) -> None:
        ...
        states_cleaned = cleanup_expired_entries(
            self._active_states, self.config.state_expiry_seconds
        )
        codes_cleaned = cleanup_expired_entries(
            self._active_codes, self.config.auth_code_expiry_seconds
        )

        if states_cleaned + codes_cleaned > 0:
            logger.debug(
                f"Cleaned up {states_cleaned} expired states and {codes_cleaned} expired codes"
            )
