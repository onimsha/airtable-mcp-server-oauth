...
class AirtableClient:
    ...
    async def _ensure_rate_limit(self) -> None:
        ...
        if self._request_count >= RATE_LIMIT_REQUESTS_PER_SECOND:
            wait_time = RATE_LIMIT_WINDOW - (current_time - self._rate_limit_window_start)
            if wait_time > 0:
    ...
    async def _make_request(
            ...
            try:
                async with httpx.AsyncClient() as client:
                    ...
                    if response.status_code == 401:
                        raise AirtableAuthError("Authentication failed - invalid or expired token")
        ...
        raise AirtableAPIError(f"Request failed after {self.max_retries + 1} attempts: {last_exception}")
    ...
    async def list_records(
        ...
        base_id: str,
        table_id: str,
        options: Optional[ListRecordsOptions] = None,
    ) -> List[AirtableRecord]:
        ...
        while True:
            ...
            if options and options.max_records and len(all_records) >= options.max_records:
    ...
    async def search_records(
        ...
        """
        logger.info(f"Searching records in {base_id}/{table_id} with formula: {filter_by_formula}")
